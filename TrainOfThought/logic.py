...
def start_game():
    ...
    train_count = state.difficulty_level * 4
    state.total_trains = train_count
    state.game_score = 0
    make_route()
    init_tracks()
...
def click_switch(x, y):
    """If the player clicks on a switch, toggle its state."""

    cell = state.grid[y][x]
    if cell is not None and cell[0] == FuncT.SWITCH:
        idx = cell[1]
        state.switches[idx][4] = not state.switches[idx][4]
...
def move_trains():
    ...
    if train_count == 0 and len(state.trains) == 0:
        state.game_active = False
        paint_btns()

    # For each currently active train
    for _ in range(len(state.trains)):
        x, y, color = state.trains.pop(0)
        cell = state.grid[y][x]
        if cell is None:
        ...
        match cell:
            ...
            case [FuncT.SWITCH, idx]:
                _, _, dir0, dir1, flag = state.switches[idx]
...
def add_trains():
    ...
    if train_count > 0:
        while True:
            color = random.randrange(state.difficulty_level)
            if color != last_color:
                state.trains.append((state.tunnel_x, state.tunnel_y, color))
                train_count -= 1
                last_color = color
                break
