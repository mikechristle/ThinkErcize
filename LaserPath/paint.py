...
count_image = pg.Surface((CELL_SIZE // 2, CELL_SIZE // 2))
...
COUNT_FONT = pg.font.SysFont('Arial', CELL_SIZE)
...
def show_click():
    ...
    pg.draw.rect(screen, DARK_GRAY, (x0, y0, w, w))
        ...
        for x in range(7):
            ...
            match st.grid[y][x]:
                ...
                case st.MIRROR2:
                    ...
                    x1 = x0 + CELL_SIZE - 20
                    y1 = y0 + CELL_SIZE - 20
                    pg.draw.line(screen, WHITE, (x0, y0), (x1, y1), width=7)
                case st.MIRROR1:
...
def init_background():
    ...
    for y in range(7):
        for x in range(7):
            ...
            x0 = (x * CELL_SIZE) + 1
            y0 = (y * CELL_SIZE) + 1
            pg.draw.rect(bg_image, color, (x0, y0, w, w))
...
def fire_laser():
    ...
    for _ in range(30):
        ...
        show_click()
        pg.draw.lines(screen, color, False, laser_path, width=5)
        show_mirrors()
        show_laser()
        pg.display.flip()
        sleep(0.1)
